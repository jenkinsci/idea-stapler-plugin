...
public class JexlInspection extends LocalXmlInspectionTool {
     ...
     * @param onTheFly
     */
    protected ProblemDescriptor[] check(
            XmlElement psi, InspectionManager manager, String text, TextRange range, boolean onTheFly) {
        if (!(psi.getContainingFile() instanceof JellyFile)) return EMPTY_ARRAY; // not a jelly script
                                    ...
                                    while (cur < len) {
                                        ...
                                        switch (c) {
                                            ...
                                            case '\'':
                                                ...
                                                break;
                                            case '}':
                                                ProblemDescriptor[] r = toArray(parseJexl(
                                                        manager,
                                                        psi,
                                                        new TextRange(cur - expr.length() - 2, cur + 1)
     ...
     *
     * @param psi PSI element that contains the given text.
     * @param range
     * @param onTheFly
     */
    private ProblemDescriptor parseJexl(
            InspectionManager manager, XmlElement psi, TextRange range, String expr, boolean onTheFly) {
        try {
            if (expr.startsWith("%")) { // property reference
                ...
                if (idx == 1) {
                    ...
                    range = new TextRange( // +3 to skip "${%"
                            range.getStartOffset() + 3, range.getStartOffset() + 4);
    ...
    private ProblemDescriptor handleParseException(
            InspectionManager manager, XmlElement psi, ParseException e, int offset, boolean onTheFly) {
        TextRange range;
        range = new TextRange(
    ...
    protected String tokenize(String text) {
        ...
        for (int idx = 0; idx < text.length(); idx++) {
            ...
            switch (ch) {
                case ',':
                    if (parenthesis == 0) return text.substring(0, idx);
                    break;
                case '(':
                case '{':
                case '[':
