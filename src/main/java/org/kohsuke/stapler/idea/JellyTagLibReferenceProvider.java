...
public class JellyTagLibReferenceProvider extends PsiReferenceProvider {
    ...
    */
    ...
    public PsiReference @NotNull [] getReferencesByElement(
            @NotNull PsiElement e, @NotNull ProcessingContext processingContext) {
        /*
        ...
        if (e instanceof XmlAttributeValue) return onAttributeValue((XmlAttributeValue) e);
    ...
    private PsiReference[] onAttributeValue(final XmlAttributeValue xav) {
        ...
        if (!(_xa instanceof XmlAttribute a)) return PsiReference.EMPTY_ARRAY;

        if (!a.getName().equals("page")) return PsiReference.EMPTY_ARRAY;

        XmlTag p = a.getParent();
        if (p == null) return PsiReference.EMPTY_ARRAY;

        if (!p.getLocalName().equals("include") || !p.getNamespace().equals("jelly:stapler"))
            return PsiReference.EMPTY_ARRAY;

        if (p.getAttribute("it") == null && p.getAttribute("from") == null) {
            // the page must be coming from the same object
            return array(new PsiReferenceBase<>(xav, TextRange.from(1, xav.getTextLength() - 2)) {
                ...
                public PsiFile resolve() {
                    ...
                    if (page.startsWith("/")) {
                        ...
                        PsiClass c = javaFacade.findClass(
                                pkg.getQualifiedName(), GlobalSearchScope.moduleWithDependenciesAndLibrariesScope(m));
